...
Flight::route('GET /api/v1/likes', [LikesController::class, 'getLikes']);
...
Flight::route('GET /api/v1/user', function() {
    $authData = AuthController::authenticate();
    $db = Flight::get('db');

    $user = $db->get('User', '*', ['id' => $authData['id']]);
    if (!$user) {
